...
import { 
  Eraser, FileText, Hash, Scissors, SquarePen, User, 
  House, Image, 
  LogOut,
  MessageSquareQuote
} from 'lucide-react'
...
const navItems = [
  ...
  { to: '/ai/social-media', label: 'Social Media', Icon: MessageSquareQuote },
        ...
        {user && (
          <>
            <img 
              src={user.imageUrl} 
              alt="user avatar" 
              className='w-18 rounded-full mx-auto'
            />
        ...
        <div className='px-6 mt-5 text-sm text-gray-600 font-medium'>
          {navItems.map(({ to, label, Icon }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 py-2.5 px-3.5 mx-3 rounded-lg mt-4 ${
                  isActive
